...
class MembersController extends AppController {
	...
	public function beforeFilter() {
		...
		if (! $this->isApi) {
			$this->Auth->allow ( 'login', 'register', 'login_fb', 'forgot_password', 'change_password', 'registercheck' ); // allow these without authentication
		}
	}
	...
	public function register() {
		...
		//		) );
				$this->render ( 'registercheck' );
			//	$this->Member->validate();
			//	$this->registercheck();
		// 2015		echo json_encode ( $this->Member->validationErrors );
		//	2015	exit ();
			}
		}
	}
	public function registercheck() {
		// $this->Session->setFlash ( __ ( 'in reg' ), 'flash/info' );
		// $this->layout = false;
		debug ( $this->request->data );
		return $this->render ( 'registercheck' );
		exit();
		// $this->render('Members/RegisterCheck');
	}
	...
	public function api_public() {
		...
		if (isset ( $data ['Member'] ['photo_updated'] ) && $data ['Member'] ['photo_updated'] > 0) {
			...
			$data ['Member'] ['profile_picture'] = null;
		}
