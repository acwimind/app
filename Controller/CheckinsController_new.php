...
class CheckinsController extends AppController {
	...
	public function api_people() {
		...
		foreach ( $result as $key => $val ) {
			...
			if ($xstatus != 'A') {
				$result [$key] ['Member'] ['surname'] = utf8_encode(substr ( $result [$key] ['Member'] ['surname'], 0, 1 ) . '.');
			}
	...
	public function api_nearbyOLD() {
		...
		foreach ( $all_nearby as $ami ) {
			...
			if ($xstatus != 'A') {
				$data ['Member'] ['surname'] = utf8_encode(substr ( $data ['Member'] ['surname'], 0, 1 ) . '.');
			}
...
//		$coords = '(' . $lon . ',' . $lat . ')';
		...
		foreach ( $all_nearby as $key => &$val ) {
                    ...
                    if ($xstatus != 'A') {
                        ...
                        $val[0]['surname'] = utf8_encode(substr ($val[0]['surname'], 0, 1 ).'.'); 
                        } 
