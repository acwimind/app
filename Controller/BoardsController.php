...
class BoardsController extends AppController {
	...
	public function api_GetBoardContent2() {
		...
		foreach ( $MySugAffinity as $key2 => &$val2 ) {
			...
			$MySugAffinity [$key2] [0] ['surname'] = mb_substr ( $MySugAffinity [$key2] [0] ['surname'], 0, 1 ) . '.';
		}
	...
	public function api_GetBoardContent() {
		// NUOVO getBoardContent SE NON FUNZIONA RINOMINARE E RIATTIVARE getBoardContentOLD
		...
		if (is_array ( $MyFriends )) {
			...
			foreach ( $MyFriends as $key => $val ) {
				...
				// ADD MEMBER PHOTO
				// debug( $val ['Member']['Member'] ['photo_updated'] );
				if (isset ( $MyFriends [$key] ['Member'] ['Member'] ['photo_updated'] ) and $MyFriends [$key] ['Member'] ['Member'] ['photo_updated'] > 0 and $MyFriends [$key] ['Member'] ['Member'] ['photosvisibility'] > 0) {
					$MyFriends [$key] ['Member'] ['Member'] ['profile_picture'] = $this->FileUrl->profile_picture ( $val ['Member'] ['Member'] ['big'], $val ['Member'] ['Member'] ['photo_updated'] );
				} else {
					$sexpic = 2;
					if (isset ( $MyFriends [$key] ['Member'] ['Member'] ['sex'] ) and $MyFriends [$key] ['Member'] ['Member'] ['sex'] == 'f') {
		...
		foreach ( $MySugAffinity2 as $key2 => $val2 ) {
			...
			$MySugAffinity2 [$key2] ['surname'] = mb_substr ( $MySugAffinity2 [$key2] ['surname'], 0, 1 ) . '.';
	...
	public function api_GetDiaryContent() {
		...
		if (is_array ( $Amici )) { // previene il warning Invalid argument supplied for foreach()
			foreach ( $Amici as &$ami ) {
				...
				$ami[0]['isvip']=$ami["Member"]["Member"]["isvip"];
				$ami[0]['isnew']=$ami["Member"]["Member"]["isnew"];
				$ami[0]['ishot']=$ami["Member"]["Member"]["ishot"];
	...
	public function api_GetRadarContent() {
		...
		foreach ( $MyPlaces as $key => $val ) {
			...
			// debug ($appollo);
			// print_r($xeve);
			$joinandcheckin = $this->Checkin->getJoinsandCheckinsCountFor ( $xeve ['Event'] ['big'] );
			$MyPlaces [$key] [0] ['CheckinsCount'] = intval ( $joinandcheckin ['checkin'] );
			$MyPlaces [$key] [0] ['JoinsCount'] = intval ( $joinandcheckin ['join'] );
		}
		...
		foreach ( $MyFriends as $key => $val ) {
			...
			if (count ( $IsFriend ) < 1) {
				...
				$MyFriends [$key] [0] ['surname'] = mb_substr ( $MyFriends [$key] [0] ['surname'], 0, 1 ) . '.';
			}
	...
	public function api_GetRadarContent_NEW() {
		...
		$time_start_1 = $this->getmicrotime (); // sec iniziali
		                                       // Prende i Places ordinati per distanza crescente da coords
		$MyPlaces = $this->Place->getRadarPlacesBoost ( $coords ); // prende i 15 place più vicini a $coords
	...
	public function api_edit() {
		...
		if (! $member) {
			$this->_apiEr ( __ ( 'Si è verificato un errore durante il salvataggio dei dati del profilo' ), true );
		}
		
		$response = array (
				'user_msg' => __ ( 'Profilo aggiornato' ) 
		);
	...
	private function _api_photo_upload() {
		$msg = ', si è verficato un errore nell\'upload della foto';
	...
	private function _save($big = 0) {
		...
		if (! empty ( $this->Member->validationErrors )) { // we have errors while saving the data
			$this->_apiEr ( __ ( 'Compilare tutti i campi obbligatori' ), true, false, array (
	...
	public function api_BoardContactsOLD() {
	...
	public function BoardContacts($ContactBIG) {
		...
		$index = array (
				$ContactBIG 
		); // previene che l'utente sia consigliato a se stesso nel caso avesse i propri
		                             // numeri in rubrica
		...
		foreach ( $data as $key => &$mem ) {
			...
			if ($AlreadyFr) { // se true allora l'amico può essere consigliato
				...
				$mem ['Member'] ['isvip'] = ($mem ['Member'] ['type'] == MEMBER_VIP);
				
				$db = $this->Member->getDataSource ();
				
				$serviceList = explode ( ',', ID_RADAR_VISIBILITY_PRODUCTS );
				$query = 'SELECT count(*) FROM wallets WHERE member1_big=' . $ContactBIG . ' AND expirationdate>NOW() AND product_id IN (' . ID_RADAR_VISIBILITY_PRODUCTS . ')';
				
				try {
					$mwal = $db->fetchAll ( $query );
					$mem ['Member'] ['ishot'] = (count ( $mwal ) > 0);
				} catch ( Exception $e ) {
				...
				$mem ['Member'] ['isnew'] = ($mem ['Member'] ['created']) > $now;
	...
	public function BoardContactsNew($ContactBIG) {
		...
		$time_start_1 = $this->getmicrotime (); // sec iniziali
		$query = "SELECT m.big,m.name,m.middle_name,m.surname,m.photo_updated,m.sex,m.phone,m.birth_date,m.address_town,m.address_country,ps.visibletousers," . "ps.photosvisibility, " . "(m.type=4) as isvip,(m.created>(now() - interval '5 days'))  as isnew," . "(SELECT COUNT(*) FROM wallets WHERE member1_big=m.big AND expirationdate>NOW() AND product_id IN (" . ID_RADAR_VISIBILITY_PRODUCTS . "))>0 AS ishot " . "FROM members m " . "JOIN privacy_settings ps ON m.big=ps.member_big " . "WHERE ( " . "m.email IN (" . "SELECT email " . "FROM contacts " . "WHERE member_big=$ContactBIG AND email IS NOT NULL) " . "OR " . 

		"m.phone IN ( " . "SELECT phone " . "FROM contacts " . "WHERE member_big=$ContactBIG AND phone IS NOT NULL) " . ") " . 
		// solo membri attivi
		"AND m.status<255 " . 
		// solo membri che non sono amici o hanno richiesto amicizia
		"AND m.big NOT IN ( " . "SELECT member1_big as friend " . "FROM friends " . "WHERE member2_big=$ContactBIG " . "UNION " . "SELECT member2_big as friend " . "FROM friends " . "WHERE member1_big=$ContactBIG ) " . 
		// solo membri non bloccati
		"AND m.big NOT IN ( " . "SELECT from_big as blocked " . "FROM member_settings " . "WHERE to_big=$ContactBIG AND chat_ignore=1 " . "UNION " . "SELECT to_big as blocked " . "FROM member_settings " . "WHERE from_big=$ContactBIG AND chat_ignore=1) " . 
		// solo membri visibili a tutti
		"AND visibletousers=1 " . "ORDER BY m.name LIMIT 500";
		...
		foreach ( $data as $key => &$mem ) {
			...
			$mem ['surname'] = mb_substr ( $mem ['surname'], 0, 1 ) . '.';
			$AppoMem [] ['Member'] = $mem;
		}
