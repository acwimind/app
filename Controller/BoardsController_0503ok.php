...
class BoardsController extends AppController {
	...
	public function api_GetBoardContent2() {
        ...
        $privacySetting=$this->PrivacySetting->getPrivacySetting($MySugAffinity[$key2]['Member']['big']);
            ...
            $MySugAffinity [$key2] [0] ['surname']= utf8_encode(substr($MySugAffinity [$key2] [0] ['surname'],0,1).'.');
    ...
    //    debug(strcmp('1.0', ANDROID_APP_VERSION)>=0);
        ...
        //print_r($MySugAffinity2);
          ...
          //   debug($val2['sex'] );
            ...
            $MySugAffinity2 [$key2] ['surname']= utf8_encode(substr($MySugAffinity2 [$key2]['surname'],0,1).'.');
//        debug( $MySugAffinity2 [$key2]['surname']);
	...
	public function api_GetRadarContent() {
		...
		foreach ( $MyFriends as $key => $val ) {
			...
			$MyFriends [$key] [0]['surname']=utf8_encode(substr($MyFriends [$key] [0]['surname'],0,1).'.');
			}
...
//		$ContactBIG = $this->api ['member_big'];
		...
		foreach ( $data as $key => &$mem ) {
			...
			if ($AlreadyFr) {//se true allora l'amico può essere consigliato
				...
				$mem ['Member']['surname']=utf8_encode(substr($mem['Member']['surname'],0,1).'.');
				$AppoMem [] = $mem;
			}
		}
     ...
     public function BoardContactsNew($ContactBIG) {
        ...
        foreach ( $data as $key => &$mem ) {
             ...
             if ($mem['photo_updated'] > 0) {
                ...
                $mem['surname']=utf8_encode(substr($mem['surname'],0,1).'.');
                $AppoMem []['Member'] = $mem;
            }
