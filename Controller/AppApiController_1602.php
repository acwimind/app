...
class AppApiController extends Controller {
	...
	protected function _checkApiToken() {
		...
		$this->_apiEr ( __("Invalid api_token and / or member_big"), __( 'There was an authentication error, try to log in again' ), false, null,'010');
		return false;
	}
...
// ENG:We are sorry but something failed. Our team will take care of the issue as soon as possible.		
	...
	protected function _apiLogin() {
		...
		if (isset ( $this->api ['email'] ) && ! empty ( $this->api ['email'] ) && isset ( $this->api ['password'] ) && ! empty ( $this->api ['password'] )) { // authenticate user account (with email and password)
			...
			if (! $member) {
				return $this->_apiEr ( __( 'Invalid email or password' ), true,null,null,'010' );
			}
			...
			if (empty ( $fb_user ) || empty ( $fb_user ['id'] )) {
				return $this->_apiEr ( __( 'Your Facebook account could not be accessed. Check your permissions for Haamble application from your Facebook.' ), true,null,null,'010' );
			}
				...
				try {
					...
					if (isset ( $this->api ['nofbmail'] ))
					{
						...
						if (!empty ( $zmember )) { // member not found based on conditions
							...
							return $this->_apiEr ( __( 'La email di questo account Facebook risulta in uso' ),  true,null,null,'298' );
						}
					}
			...
			return $this->_apiEr ( __( 'Please specify email or password' ), true,null,null,'010' );
		}
