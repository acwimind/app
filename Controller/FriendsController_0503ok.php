	...
	public function api_GetFriendsTest() {
        ...
        foreach ( $Amici as $key=>$ami ) {
            ...
            if ($ami['member1_big'] == $idMember) {
                ...
                if ($ami['status']!='A')
                {
                    $ami['surname'] = utf8_encode(substr ( $ami['surname'], 0, 1 ) . '.');
                }
                //$xami[] = $ami;
            } 
    ...
    public function api_GetFriendsOLD() {
		...
		foreach ( $Amici as $ami ) {
			...
			if ($ami ["Friend1"] ["big"] == $idMember) {
				...
				{
					$ami ["Friend2"] ['surname'] = utf8_encode(substr ( $ami ["Friend2"] ['surname'], 0, 1 ) . '.');
				}
				$xami [] = $ami ["Friend2"];
			} 
				...
				if ($ami ["Friend"] ["status"]!='A')
				{
					$ami ["Friend1"] ['surname'] = utf8_encode(substr ( $ami ["Friend1"] ['surname'], 0, 1 ) . '.');
				}
				$xami [] = $ami ["Friend1"];
			}
