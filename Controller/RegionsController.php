...
class RegionsController extends AppController {
	...
	public function admin_index() {
		...
		$this->_savedFilter(array('srchphr', 'country'));

		$conditions = array();
	    	
    	if (isset($this->params->query['srchphr']) && !empty($this->params->query['srchphr'])) {
    		$conditions['OR'] = array('Region.city ILIKE' => '%' . $this->params->query['srchphr'] . '%') ;
    	}
	...
	public function admin_edit($id=0) {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if ($this->Region->saveAll($this->request->data, array('validate' => 'first'))) {
	...
	public function admin_delete($id) {
		...
		if ($has_places == 0) {
			$this->Region->RegionLang->deleteAll(array('RegionLang.region_id' => $id));
...
//		}
		...
		$this->_apiOk(array('cities' => $cities));
		
	}

	public function autocomplete($city=null, $country=null) {
		
		/*if (!$this->isAjax) {	//allow only ajax access
