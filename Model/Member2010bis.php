...
//        ),
    ...
    for ($i=0; $i<count($distanceAttempts); $i++){ //distance array iterator index
       ...
       $sql2 = "SELECT members.big,members.name,members.surname,members.updated,members.birth_date,members.photo_updated,members.sex,".
               "members.last_lonlat AS \"coordinates\",((members.last_lonlat <@> ? )::numeric(10,1) * 1.6) AS \"distance\" ".               "FROM members ".
               "WHERE ((".$Iyear." - DATE_PART('year', birth_date)) BETWEEN -".$ageAttempts[$j]." AND ".$ageAttempts[$j].") ".
               "AND sex != '".$Imember['sex']."' ".
     ...
     if (!count($result)>=MIN_AFFINITY_MEMBERS){// extreme attempt : max age diff, max distance and any members sex
         ...
         $sql2 = "SELECT members.big,members.name,members.surname,members.updated,members.birth_date,,members.photo_updated,members.sex,".
                  ...
                  "WHERE (( ".$Iyear." - DATE_PART(\'year\', birth_date)) BETWEEN -".$ageAttempts[count($ageAttempts)-1].
