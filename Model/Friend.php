...
class Friend extends AppModel {
	...
	public function getBoardFriends($MemberID) {
		...
		foreach ($Amici as $ami) {
			...
			{
				$FriendsID.= $ami["Friend2"]["big"].',';
			}
					
				else
				{
				$FriendsID.= $ami["Friend1"]["big"].',';
	...
	public function FriendsRelationshipGeneric($memberOne, $memberTwo) {
		...
		$params = array (
				'conditions' => array (
						'AND' => array (
								'OR' => array (
										array ('Friend.member1_big' => $memberOne),
										array ('Friend.member2_big' => $memberOne)
								),
								'OR' => array (
										array ('Friend.member1_big' => $memberTwo),
										array ('Friend.member2_big' => $memberTwo)
								)
				)
		));
