...
//TODO: rimetterlo??		'ssl://gateway.sandbox.push.apple.com:2195' // Sandbox environment
	...
	public function send()
	{
		...
		while (($nMessages = count($this->_aMessageQueue)) > 0) {
			...
			if (!$bError) {
				...
				if ($nChangedStreams === false) {
					...
					$this->_log("INFO: Sending messages - changedStreams > 0");
					$bError = $this->_updateQueue();
					if (!$bError) {
						$this->_log("INFO: Sending messages - changedStreams > 0 - emptying");
						$this->_aMessageQueue = array();
					}
				} else {
					$this->_log("INFO: Sending messages - emptying");
					$this->_aMessageQueue = array();
				}
			}
		...
		$this->_log("INFO: Sending messages procedure finished");
	}
