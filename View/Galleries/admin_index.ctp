...
if (isset ( $data ['Event'] ['big'] ) && $data ['Event'] ['big'] > 0) {
	echo __ ( 'event %s at ', $this->Html->link ( $data ['Event'] ['name'], array (
			'controller' => 'events',
			'action' => 'index',
			'?' => array (
					'srchname' => $data ['Event'] ['name'],
					'srchplace' => $data ['Place'] ['name'] 
			) 
	) ) );
}

echo __ ( 'place %s', $this->Html->link ( $data ['Place'] ['name'], array (
		'controller' => 'places',
		'action' => 'index',
		'?' => array (
				'srchphr' => $data ['Place'] ['name'] 
		) 
) ) ) . '</h2>';

if (isset ( $data ['Event'] ['big'] ) && $data ['Event'] ['big'] > 0) {
	echo $this->Html->link ( __ ( 'Back to event edit' ), array (
			'controller' => 'events',
			'action' => 'edit',
			$data ['Event'] ['big'] 
	) );
	...
	echo $this->Html->link ( __ ( 'Back to place edit' ), array (
			'controller' => 'places',
			'action' => 'edit',
			$data ['Place'] ['big'] 
	) );
...
if ($fc == 'event_photo') {
	echo $this->AdvForm->create ( 'Event', array (
			'url' => array (
					'controller' => 'events',
					'action' => 'upload',
					$data ['Event'] ['big'] 
	...
	echo $this->AdvForm->create ( 'Place', array (
			'url' => array (
					'controller' => 'places',
					'action' => 'upload',
					$data ['Place'] ['big'] 
...
foreach ( $data ['Photo'] as $photo ) {
	...
	echo $this->Html->link ( '<i class="icon-star-empty' . $icon_class . '"></i> ' . __ ( 'Set as default' ), array (
			'controller' => 'photos',
			'action' => 'default',
			$photo ['big'] 
	), array (
